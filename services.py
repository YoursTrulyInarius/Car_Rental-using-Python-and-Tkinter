...
class CarRentalService:
    ...
    def adjust_vehicle_stock(self, session, make, model, year, current_reg, daily_rate, target_qty):
        ...
        if target_qty > current_qty:
            ...
            # Find base registration to increment
            # Heuristic: Try to match the style of the current_reg or use it as base
            base_reg = current_reg.split('-')[0] + "-" + current_reg.split('-')[1] if '-' in current_reg else current_reg
            
            # Find highest existing suffix
            max_suffix = 0
            for v in existing_vehicles:
                parts = v.registration.split('-')
                if len(parts) > 2 and parts[-1].isdigit():
                    s = int(parts[-1])
                    if s > max_suffix: max_suffix = s
            ...
            for i in range(needed):
                ...
                # Double check it doesn't exist (unlikely but safe)
                if session.query(Vehicle).filter_by(registration=new_reg).first():
                    new_reg = f"{new_reg}-N" # Fallback
            ...
            # Remove the last added ones first (heuristic)
            # Sort by ID desc
    ...
    def create_rental(self, session, customer_id, vehicle_id, return_date_str, rental_date_str=None):
        ...
        if duration <= 0:
            return None, "Return date must be in the future."
